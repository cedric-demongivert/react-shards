...
export function isPluginStorePush (createStore) {
  ...
  it('is mutable if was declared as mutable', function () {
    ...
    if (!firstStore.isImmutable()) {
      ...
      expect(firstStore.get('endpoints.first')).to.be.eql([1, 2, 3, 4, 5])
      expect(thirdStore.get('endpoints.first')).to.be.eql([1, 2, 3, 4, 5])
      expect(secondStore.get('endpoints.first')).to.be.eql([1, 2, 3, 4, 5])
      expect(lastStore.get('endpoints.first')).to.be.eql([1, 2, 3, 4, 5])
      expect(firstStore.get('endpoints.second')).to.be.eql([3, 4, 5])
      expect(secondStore.get('endpoints.second')).to.be.eql([3, 4, 5])
      expect(thirdStore.get('endpoints.second')).to.be.eql([3, 4, 5])
      expect(lastStore.get('endpoints.second')).to.be.eql([3, 4, 5])
    }
  })
}
